// frontend/src/components/BookingCalendar.jsx - NEW COMPONENT
...
const BookingCalendar = ({ service, onBookingComplete }) => {
  ...
  const fetchAvailableSlots = async (date) => {
    ...
    try {
      ...
      // Use the correct endpoint for availability
      const response = await api.get(
        `/bookings/available-slots/${serviceId}?date=${date}`
      );
      const slots = response.data?.slots || [];
      if (Array.isArray(slots) && slots.length > 0) {
        // If slots are objects with start/end, convert to time strings
        if (typeof slots[0] === 'object' && slots[0].start) {
          const timeSlots = slots
            ...
            .filter(Boolean);
          setAvailableSlots(timeSlots.length > 0 ? timeSlots : generateDefaultTimeSlots());
        } else {
          // Slots are already strings or simple values
          setAvailableSlots(slots.filter(Boolean));
        }
      } else {
        // No slots returned, generate defaults
        generateDefaultSlots();
      }
    } catch (error) {
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      // Pass booking data to parent for checkout
      if (onBookingComplete) {
        await onBookingComplete(bookingData);
      } else {
        toast.error("Booking handler not available");
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      {service && (
        <div className="p-4 bg-purple-50 dark:bg-purple-950 rounded-lg border border-purple-200 dark:border-purple-800">
          <h3 className="font-bold text-lg mb-2">{service.title || "Service"}</h3>
        ...
        <div className="space-y-2">
          ...
          {loadingSlots ? (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-3 text-muted-foreground">Loading available slots...</span>
            </div>
              ...
              {availableSlots.map((slot) => (
                <button
                  ...
                  className={`px-4 py-3 rounded-lg border-2 transition-all ${
                    selectedTime === slot
                      ? "border-purple-600 bg-purple-50 dark:bg-purple-950 text-purple-700 dark:text-purple-300 font-semibold"
                      : "border-slate-200 dark:border-slate-700 hover:border-purple-300 hover:bg-purple-50/50 dark:hover:bg-purple-950/50"
                  }`}
                >
                  {slot}
                </button>
      ...
      {/* Duration */}
      <div className="space-y-2">
        <label className="block font-semibold">Session Duration</label>
        <select
          value={duration}
        ...
        <div className="p-4 bg-blue-50 dark:bg-blue-950 rounded-lg border border-blue-200 dark:border-blue-800">
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="text-sm text-blue-600 dark:text-blue-500">
                <strong>Total:</strong> ${service?.price || 0}
              </p>
            </div>
          </div>
        </div>
      ...
      <button
        ...
        className="w-full h-12 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-xl hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
      >
        {loading ? (
          <>
